...
let self = module.exports = {
    ...
    ghclean: function(data) {
        let dir = data.repoPath + data.deploy.gh.cache;
        return self.clean(dir);
    },
  ...
  open: function(data) {
      Debug.L1('opening ' + data.build.url + ' in ' + data.browser);
      if (Array.isArray(data.browser)) {
          for (let browser of data.browser) {
              viewinbrowser(data.build.url, browser);
          }
      } else {
          viewinbrowser(data.build.url, data.browser);
      }
  }
}
