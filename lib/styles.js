...
let self = module.exports = {
    ...
    // TODO add minify when doing production build
    merge: function(data) {
        ...
        return new Promise(function(resolve, reject) {
            fstream.src(src, {
                ...
                .on('end', function() {
                    Debug.L1('CSS Concatenated');
                })
                // convert this to minify, autoprefix, etc.
                //    .pipe(pif(data.build.type === 'dev', sourcemaps.write('.')
                //        .on('end', function() {Debug.L1('SourceMaps Written...');})
                //          ))
                .pipe(fstream.dest(dest))
                .on('end', function(resolve) {
                    Debug.L1('CSS written to ' + dest);
                    // TODO deal with merging sourcemaps
                    // sorcery.load(dest + 'site.css').then( chain => {
                    // chain.write().then(resolve('concat the sourcemaps'))
                    //})
