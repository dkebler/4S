...
let browserify = require('browserify'),
  // for stream
    ...
    mainBowerFiles = require('wiredep')
    ;
...
let self = module.exports = {
        ...
        bowerDeps: function(data) {
            return new Promise(function(resolve, reject) {
             ...
             // let src = mainBowerFiles('**/*.js'); // using main-bower-files
             // if (src.length > 0) {

             let src = mainBowerFiles();   // using wiredep
             if (src.js) {   // using wiredep

              fstream.src(src.js)
              // fstream.src(src)  //using main-bower-files
                      .pipe(concat('bowerDeps.js'))
              //      .pipe(uglify())  // for development
                      .pipe(fstream.dest(data.repoPath + data.dir[data.build.type] + 'assets/'))
                      .on('end', resolve); //end of stream
             }
              else {
                Debug.L1('no js bower files to reference');
